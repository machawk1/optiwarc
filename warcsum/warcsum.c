...
process_file (char *in, FILE* f_out, z_stream* z, struct warcsum_struct* ws)
{
  ...
  fprintf(stderr, "File size by stat(2) = %lu\n", file_size);
  //fseek (f_in, 0, SEEK_END);
  //file_size = ftello(f_in);
  //fprintf(stderr, "File size by ftell(3) = %li\n", file_size);
  ...
  while (ftello(f_in) < file_size || ws->need_double);
